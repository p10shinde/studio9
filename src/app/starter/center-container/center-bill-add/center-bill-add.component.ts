...
export class CenterBillAddComponent implements OnInit {
  ...
  resetBill(discountCheckBox: NgModel, advancepaycheck: NgModel, customerSelect: NgModel) {
    this.passportItemList = this.preWeddingItemList = this.weddingItemList = this.birthdayItemList =
    this.customItemList = this.cartItems = [];
  ...
  chck(listName: string, refItem: any,  qtyInput: NgModel, listTotal: string) {
  ...
  saveBill() {
    ...
    if (this.custID) {
      if (this.cartItems.length > 0) {
        ...
        if (!this.billNo) {
          this.billService.saveBill({userID, custID, userContact, billTotal, advancePayment, pendingPayment,
                                    pPaymentType, aPaymentType, billDetails, billDiscountedTotal,
          ...
          this.modifyBill({userID, custID, userContact, billTotal, advancePayment, pendingPayment,
                            pPaymentType, aPaymentType, billDetails, billDiscountedTotal,
                            discount, custName, custContact, userFullName });
        }
      } else {
